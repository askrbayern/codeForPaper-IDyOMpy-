...
if ~nargout || plotflag
    ...
    figure(p.fig2); clf;    
    subplot 121
    [pxx,f]=nt_spect_plot(x,p.nfft,[],[],1/fline);
    divisor=sum(pxx);
    ...
    set(hh(1),'color','k')
    subplot 122
    [pxx,f]=nt_spect_plot(y,p.nfft,[],[],1/fline);
    semilogy(f,abs(pxx)/divisor);
    hold on
    [pxx,f]=nt_spect_plot(x-y,p.nfft,[],[],1/fline);
    semilogy(f,abs(pxx)/divisor);
    legend('clean', ['removed (', num2str(nt_wpwr(x-y)/nt_wpwr(nt_demean(x)),'%.2g'),')']); legend boxoff
    set(gca,'ygrid','on','xgrid','on');
...
%prop_removed=nt_wpwr(x-y)/nt_wpwr(nt_demean(x));
